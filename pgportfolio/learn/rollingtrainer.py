...
#uses as input TraderTrainer class
class RollingTrainer(TraderTrainer):
    ...
    #returns coin list from datamatrices
    @property
    def coin_list(self):
        return self._matrix.coin_list

    #returns datamatrices panel
    @property
    def data_matrices(self):
        return self._matrix

    #returns from net_config.json the rolling training steps
    @property
    def rolling_training_steps(self):
        return self.config["trading"]["rolling_training_steps"]

    #does nothing if fast train is true, not implemented yet
