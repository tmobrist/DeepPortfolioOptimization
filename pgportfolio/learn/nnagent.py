...
#NNagent used during training from tradertrainer. Initiaizes NN, loss function, diverse values and evaluation of NN
class NNAgent:
    ...
    #closes session, resets default graph
    def recycle(self):
    ...
    #used during Initalization, initalizes the training method
    def init_train(self, learning_rate, decay_steps, decay_rate, training_method):
        ...
        if training_method == 'GradientDescent':
            ...
            gradients, variables = zip(*optimizer.compute_gradients(self.__loss))
            gradients, _ = tf.clip_by_global_norm(gradients, 5.0)
            train_step = optimizer.apply_gradients(zip(gradients, variables), global_step=self.__global_step)
    ...
    #used for every mini batch, returns set w at
    ...
    #used by trader during backtest, the history is a 3d mjatrix, return new asset vector
    def decide_by_history(self, history, last_w):
        ...
        #adds new axis to X, which was the batch axis
        history = history[np.newaxis, :, :, :]
        #returns new asset vector
