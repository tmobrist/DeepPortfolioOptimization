...
def fill_layers_default(layers):
    for layer in layers:
        if layer["type"] == "ConvLayer_over_features":
        ...
        elif layer["type"] == "ConvLayer_over_window":
            set_missing(layer, "padding", "valid")
            set_missing(layer, "strides", [1, 1])
            set_missing(layer, "activation_function", "relu")
            set_missing(layer, "regularizer", None)
...
#used in main, returns net_config.json, if index a numer --> returns subfolder net_config, returns preprocess_config
#--> uses default setting to initialize net_config complete
