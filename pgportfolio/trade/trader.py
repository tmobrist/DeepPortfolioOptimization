...
class Trader:
    def __init__(self, waiting_period, config, total_steps, net_dir, agent=None, initial_BTC=1.0, agent_type="nn"):
        ...
        if agent_type == "traditional":
            ...
            #gets coinlist from datamatrices
    ...
    #opens generate history matrix in BackTest, returns X of the test set until the current time point
    ...
    #used by start_trading in every trading step
    ...
    #startet by tradertrainer or shortcut during backtest, returns trade body for every step
    def start_trading(self):
        try:
            if not self.__class__.__name__=="BackTest":
                ...
                #self._steps initialized as 0, counts up to total steps
                while self._steps < self._total_steps:
                    self.__trade_body()
        finally:
            if self._agent_type=="nn":
