...
def build_parser():
    ...
    parser.add_argument("--format", dest="format", default="raw",
                        help="format of the tab22 printed")
    ...
    parser.add_argument("--date", dest="date",
                        help="date range for plotting portfolio weights sep. by; and in the format %Y_%m_%d_%H_%M"
                        "Possibility: --mode=plot --algos=4,6 --labels=lw_in_lf_2015/10/15_2017/10/15,lw_in_nn_2015/10/15_2017/10/15 --date=2017_9_12_0_0;2017_9_19_0_0 --coinlist=DGB,BTS,reversed_USDT")
    parser.add_argument("--coinlist", dest="coinlist", help="list of coins, which should be plotted in the porftolio weights graph")
    return parser
...
def main():
    ...
    if options.mode == "train":
        ...
        #load_config returns preprocess_config(config)
        generate.add_packages(load_config(), int(options.repeat))
    elif options.mode == "download_data":
        from pgportfolio.marketdata.datamatrices import DataMatrices
        with open("./pgportfolio/net_config.json") as file:
            config = json.load(file)
        #preprocess checks the python version and loads default config setting
    ...
    #returns plot of portfolio value - time
        ...
        if options.labels:
            ...
            labels = algos
        if options.date:
            datess = options.date.replace("_"," ")
            datess = datess.split(";")
        else:
            datess = None
        if options.coinlist:
            coinlist = options.coinlist.split(",")
        else:
            coinlist = None
        plot.plot_backtest(load_config(), algos, labels, datess, coinlist)
